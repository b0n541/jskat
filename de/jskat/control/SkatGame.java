...
public class SkatGame extends JSkatThread {
	 ...
	 *            Hind hand player
	 */
	public SkatGame(JSkatMaster jskat, String newTableName, 
			JSkatPlayer newForeHand,
	...
	public void dealCards() {
		...
		for (int i = 0; i < 3; i++) {
			...
			case 0:
				...
				this.data.setSkatCards(this.deck.remove(0), this.deck.remove(0));
				break;
			case 1:
				// deal four cards
				dealCards(4);
		...
		log.debug("Fore hand: " + this.data.getPlayerCards(Player.FORE_HAND));
		log.debug("Middle hand: " + this.data.getPlayerCards(Player.MIDDLE_HAND));
		log.debug("Hind hand: " + this.data.getPlayerCards(Player.HIND_HAND));
	...
	private void bidding() {
		...
		bidOrderIndex = twoPlayerBidding(Player.MIDDLE_HAND, 
											Player.FORE_HAND,
											bidOrderIndex);
		...
		if (this.data.getBidValue() > 0 && firstWinner == Player.MIDDLE_HAND) {
			...
			// if at least a bid was announced and middle hand has won the bidding
			bidOrderIndex++;
		}
		
		bidOrderIndex = twoPlayerBidding(Player.HIND_HAND, 
											firstWinner, bidOrderIndex);
	 ...
	 * @param startBidOrderIndex Index of first bid value from bid order array
	 * @return
	 */
	private int twoPlayerBidding(Player announcer, Player hearer,
		...
		while (!announcerPassed && !hearerPassed) {
			...
			// ask player
			int announcerBidValue = this.player[announcer.getOrder()].bidMore(nextBidValue);
			
			if (announcerBidValue > -1) {

				log.debug("announcer holds " + announcerBidValue);
				...
				if (this.player[hearer.getOrder()].holdBid(announcerBidValue)) {
					...
					log.debug("hearer holds " + announcerBidValue);
	...
	private void discarding() {
		...
		this.view.addCard(tableName, this.data.getDeclarer(), this.data.getSkat().get(0));
		this.view.addCard(tableName, this.data.getDeclarer(), this.data.getSkat().get(1));
		...
		this.view.removeCard(tableName, this.data.getDeclarer(), discardedSkat.get(0));
		this.view.removeCard(tableName, this.data.getDeclarer(), discardedSkat.get(1));
	}
	...
	private void announceGame() {
		...
		try {
			...
			GameAnnouncement ann = (GameAnnouncement)this.declarer.announceGame().clone();
	...
	private void playTricks() {
		...
		for (int i = 0; i < 10; i++) {
			...
			if (i == 0) {
				...
				this.trickForeHand = this.data.getTrickWinner(this.data.getTricks().size() - 1);
			}
			...
			playCard(this.rules, trick, this.trickForeHand);
			doSleep(this.maxSleep);
			
			log.debug("middle hand plays"); //$NON-NLS-1$
			playCard(this.rules, trick, this.trickForeHand.getLeftNeighbor());
			doSleep(this.maxSleep);
			
			log.debug("hind hand plays"); //$NON-NLS-1$
			playCard(this.rules, trick, this.trickForeHand.getRightNeighbor());
			...
			log.debug("Points: fore hand: " + this.data.getPlayerPoints(Player.FORE_HAND) + //$NON-NLS-1$
						" middle hand: " + this.data.getPlayerPoints(Player.MIDDLE_HAND) + //$NON-NLS-1$
						" hind hand: " + this.data.getPlayerPoints(Player.HIND_HAND)); //$NON-NLS-1$
	...
	private void playCard(BasicSkatRules rules, Trick trick, Player currPlayer) {
		...
		else if (!rules.isCardAllowed(this.data.getGameType(), trick.getFirstCard(),
						this.data.getPlayerCards(currPlayer), card)) {
			// TODO throw appropriate exception
	...
	private void calculateGameValue() {
		...
		log.debug("game value=" + this.data.getResult() + ", bid value="
		...
		for (JSkatPlayer currPlayer : this.player) {
			// no cloning neccessary because all parameters are primitive data types
			currPlayer.setGameResult(this.data.isGameWon(), this.data.getGameResult());
	...
	public void setGameAnnouncement(GameAnnouncement ann) {
		...
		for (JSkatPlayer currPlayer : this.player) {
			...
			currPlayer.startGame(this.data.getDeclarer(),
				 this.data.getGameType(),
				 this.data.isHand(),
				 this.data.isOuvert(),
				 this.data.isSchneiderAnnounced(),
