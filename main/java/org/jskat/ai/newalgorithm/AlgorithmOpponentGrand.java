...
public class AlgorithmOpponentGrand extends AbstractAlgorithmAI {
	...
	public static Card playStartGameCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation, Player pDeclarer) {
		return playForehandCard(pCards, pTrickCards, pPlayedCards, pNotOpponentCards, pSituation, pDeclarer);
	}
	...
	public static Card playForehandCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation, Player pDeclarer) {
		...
		for(Suit s : Suit.values()) {
			...
			// Wenn hoechste Karte und weniger als 5 Karten bislang gespielt wurden -> spielen
			if(Helper.isHighestSuitCard(possibleHighCard, pPlayedCards, pTrickCards)
					&& pPlayedCards.getSuitCount(s, false) < 5) {
			...
			else if(!tDeclarerInMiddle && pSituation.isLeftPlayerBlankOnColor(s))
				return possibleHighCard;
		}
	...
	public static Card playMiddlehandCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation, Player pDeclarer) {
		...
		if(tDeclarerInForehand) {
			...
			if(tForehandCard.getRank() == Rank.JACK) {
				...
				if(pCards.get(0).getRank() == Rank.JACK) {
					// Wenn schlagbar, dann den niedrigsten Buben spielen, der die Vorhand schlaegt
					if(pCards.get(0).beats(GameType.GRAND, tForehandCard)) {
						if(pCards.get(1).beats(GameType.GRAND, tForehandCard)) {
							if(pCards.get(2).beats(GameType.GRAND, tForehandCard)) {
								return pCards.get(2);
							}
							return pCards.get(1);
						}
				...
				// Wenn der Mitspieler noch einen Buben haben koennte, der besser ist
				if(Helper.countJacks(pNotOpponentCards) != 3
						&& ((!pNotOpponentCards.contains(Card.CJ) && tForehandCard != Card.CJ)
							|| (pNotOpponentCards.contains(Card.CJ) && !pNotOpponentCards.contains(Card.SJ) && tForehandCard != Card.SJ))) {
					CardList tPossibleCards				= new CardList();
					// Butter eine hohe Karte in der Farbe in der der Solo-Spieler blank ist
					for(Suit s : pSituation.getRightPlayerBlankSuits()) {
						if(pCards.getSuitCount(s, false) == 0)
							continue;
						Card lPossibleHighCard	= pCards.get(pCards.getFirstIndexOfSuit(s, false));
			...
			// Wenn Farbe gespielt
				...
				// Wenn die Karte die Hoechste ist || Mitspieler keine Karte der Farbe hat -> Lusche werfen
				if(Helper.isHighestSuitCard(tForehandCard, pSituation.getGameType(), pPlayedCards, pTrickCards)
					...
					return getLowValueCard(pCards, pSituation, tForehandCard, pPlayedCards, pTrickCards);
				}
			}
			// Blank Suites vom Solo-Spieler freispielen
			for(Suit s : pSituation.getRightPlayerBlankSuits()) {
				Card lPossibleLowCard	= pCards.get(pCards.getLastIndexOfSuit(s, false));
				if(lPossibleLowCard.getPoints() <= 4) {
		...
		// Solo-Spieler sitzt hinter dem Spieler
			...
			// Solo-Spieler ist blank auf der Farbe oder hat eine moegliche Karte -> wahrscheinlich gedrueckt!
			if(pSituation.getLeftPlayerBlankSuits().contains(tSuit) || pNotOpponentCards.getSuitCount(tSuit, false) >= 6)
			...
			if(Helper.isHighestSuitCard(tForehandCard, pPlayedCards, pTrickCards)) {
				...
				if(pSituation.isLeftPlayerBlankOnColor(tSuit)) {
					...
					if(Helper.countJacks(pPlayedCards)+Helper.countJacks(pCards) == 4 || !pSituation.leftPlayerHasTrump()) {
				...
				// Solo-Spieler hat moeglicherweise noch eine Karte der Farbe -> Buttern
				else {
					return getHighValueCard(pCards, pSituation, tForehandCard);
				}
			}
...
//			tDeclarerBlankSuits				= pSituation.getRightPlayerBlankSuits();
		...
		if(tForehandCard.getRank() == Rank.JACK) {
			...
			if(pCards.get(0).getRank() == Rank.JACK) {
				// Wenn der Stich dem Solo-Spieler gehÃƒÂ¶rt -> versuchen zu stechen
		...
		// Farbe gespielt
			...
			// Muss bedienen
			if(pCards.hasSuit(pSituation.getGameType(), tForehandCard.getSuit())) {
				Card possibleHighCard	= pCards.get(pCards.getFirstIndexOfSuit(tForehandCard.getSuit(), false));	// highest Card
				Card possibleLowCard	= pCards.get(pCards.getLastIndexOfSuit(tForehandCard.getSuit(), false));	// lowest Card
			...
			// Solospieler hat den Stich
				...
				// Lusche werfen
			if(tMiddlehandCard.beats(pSituation.getGameType(), tForehandCard) == (pPlayerPosition.getRightNeighbor() == pDeclarer)) {
				if(pCards.get(0).getRank() == Rank.JACK && pTrickCards.getTotalValue() > 9)
					return pCards.get(Helper.countJacks(pCards)-1);
				return getLowValueCard(pCards, pSituation, tForehandCard, pPlayedCards, pTrickCards);
			}
	...
	// Buttern
	public static Card getHighValueCard(CardList pCards, Situation pSituation, Card pInitialCard) {
		CardList possibleCards				= new CardList();
		...
		if(!possibleCards.isEmpty())
			...
			return getRandomAllowedCard(pCards, pInitialCard, pSituation.getGameType());
	}
	
	// Lusche Werfen
	public static Card getLowValueCard(CardList pCards, Situation pSituation, Card pInitialCard, CardList pPlayedCards, CardList pTrickCards) {
		CardList possibleCards				= new CardList();
		...
		for(Suit s : Suit.values()) {
			...
			if(lSuitCount == 1 && possibleHighCard.getPoints() < 10)
				return possibleHighCard;
			if(lSuitCount == 2 && Helper.isHighestSuitCard(possibleHighCard, pPlayedCards, pTrickCards))
				return possibleLowCard;
			if(lSuitCount >= 3)
				possibleCards.add(possibleLowCard);
		}
