...
class BidEvaluator {
	...
	BidEvaluator(CardList pCards, Player pPlayersPosition) {
		...
		oLog.debug("Checking out what to bid with [" + pCards + "]" + pCards.dumpFlag());
		eval(pCards);
	}
...
//			oMaxBid				= tMultiplier * SkatConstants.getGameBaseValue(GameType.GRAND, false, false);
//		}
//		if(check4Null(pCards)) {
//			int tMaxBid			= SkatConstants.getGameBaseValue(GameType.NULL, oNullHand, oNullOuvert);
//			
//			if(tMaxBid > oMaxBid) {
...
//		}
	...
	private boolean check4Grand(CardList pCards) {
		...
		// Count Blanksuits
		int tBlankSuits	= 0;
		if(pCards.getSuitCount(Suit.CLUBS, false) == 0)		tBlankSuits++;	// Clubs
		if(pCards.getSuitCount(Suit.SPADES, false) == 0)	tBlankSuits++;	// Spades
		if(pCards.getSuitCount(Suit.HEARTS, false) == 0)	tBlankSuits++;	// Hearts
		if(pCards.getSuitCount(Suit.DIAMONDS, false) == 0)	tBlankSuits++;	// Diamonds
		...
		if((pCards.contains(Card.CJ) && pCards.contains(Card.SJ) && oPlayersPosition.equals(Player.FOREHAND) && tFluteCount+tJackCount == 9) ||
				(pCards.contains(Card.CJ) && pCards.contains(Card.SJ) && tJackCount >= 3 && tFluteCount+tJackCount == 10)) {
			oGrandHand		= true;
		...
		if((pCards.contains(Card.CJ) && pCards.contains(Card.SJ) && oPlayersPosition.equals(Player.FOREHAND) && tFluteCount+tJackCount == 9) ||
				(pCards.contains(Card.CJ) && pCards.contains(Card.SJ) && tJackCount >= 3 && tFluteCount+tJackCount == 9)) {
			oGrandHand		= true;
			oGrandSchneider	= true;
			return true;
		}
		if(tAcesCount+t10DuoCount >=3
				|| (tAcesCount+t10DuoCount == 2 && ((tBlankSuits >= 1 && tJackCount >= 2 && pCards.get(1) == Card.SJ) || (tFluteCount >= 4 && tJackCount >= 3) || (tFluteCount >= 5 && pCards.contains(Card.CJ) && pCards.contains(Card.SJ))))
				|| tFluteCount >= 7
				|| (tFluteCount >= 2 && tBlankSuits == 2))
			return true;
	...
	private void getHighestSuitMultiplier(CardList pCards) {
		...
		int tMultiplier 			= Helper.getSuitMultiplier(pCards, tMostFrequentSuitColor);
		...
		for(Suit s : Suit.values()) {
			if(pCards.getSuitCount(s, false) == 0)
				tBlankCount++;
			else if(pCards.getSuitCount(s, false) == 1 && !pCards.contains(Card.getCard(s, Rank.ACE)))
				t1CardCount++;
			else if(pCards.getSuitCount(s, false) == 2 && !pCards.contains(Card.getCard(s, Rank.ACE)) && !pCards.contains(Card.getCard(s, Rank.TEN)))
				t2CardCount++;
		}
		...
		int tWinCardCount			= 0;
		if(tMostFrequentSuitColor != Suit.CLUBS		&& (pCards.contains(Card.CA) || (pCards.contains(Card.CT) && pCards.getSuitCount(Suit.CLUBS, false) >= 2)))		tWinCardCount++;	// Clubs
		if(tMostFrequentSuitColor != Suit.SPADES	&& (pCards.contains(Card.SA) || (pCards.contains(Card.ST) && pCards.getSuitCount(Suit.SPADES, false) >= 2)))	tWinCardCount++;	// Spades
		if(tMostFrequentSuitColor != Suit.HEARTS	&& (pCards.contains(Card.HA) || (pCards.contains(Card.HT) && pCards.getSuitCount(Suit.HEARTS, false) >= 2)))	tWinCardCount++;	// Hearts
		if(tMostFrequentSuitColor != Suit.DIAMONDS	&& (pCards.contains(Card.DA) || (pCards.contains(Card.DT) && pCards.getSuitCount(Suit.DIAMONDS, false) >= 2)))	tWinCardCount++;	// Diamonds
		...
		if(tNumberOfTrumpCards >= 8 && pCards.contains(Card.CJ)) {
			if(tWinCardCount > 0)
				oSuitSchneider		= true;
			oSuitHand				= true;
			int tMaxBid				= (tMultiplier+1) * SkatConstants.getGameBaseValue(GameType.valueOf(tMostFrequentSuitColor.longString().toUpperCase()), false, false);
			if(tMaxBid > oMaxBid) {
				oSuggestedGameType	= GameType.valueOf(tMostFrequentSuitColor.longString().toUpperCase());
		...
		// Wenn mit 4 und Ass
				...
				|| (tNumberOfTrumpCards >= 5 && tWinCardCount >= 1 && (tBlankCount+t1CardCount > 0))
				|| (tNumberOfTrumpCards >= 6 && (tWinCardCount >= 1 || ( t1CardCount == 2 && t2CardCount == 1)))) {
			int tMaxBid				= tMultiplier * SkatConstants.getGameBaseValue(GameType.valueOf(tMostFrequentSuitColor.longString().toUpperCase()), false, false);
			if(tMaxBid > oMaxBid) {
				oSuggestedGameType		= GameType.valueOf(tMostFrequentSuitColor.longString().toUpperCase());
				oSuggestedTrumpSuit	= tMostFrequentSuitColor;
				oMaxBid				= tMaxBid;
	...
	private int getSuitNullLength(CardList pCards, Suit pSuit) {
		...
		if(pCards.contains(Card.getCard(pSuit, Rank.SEVEN))) {
			...
			if(pCards.contains(Card.getCard(pSuit, Rank.EIGHT))) {
				...
				tNullLength++;
				if(pCards.contains(Card.getCard(pSuit, Rank.EIGHT))) tNullLength++;
				if(pCards.contains(Card.getCard(pSuit, Rank.NINE))) tNullLength++;
			}
		}
		return tNullLength;
	}
