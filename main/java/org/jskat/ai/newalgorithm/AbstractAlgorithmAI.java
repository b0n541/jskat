...
public abstract class AbstractAlgorithmAI {
	...
	public Card playCard() {
		...
		if((currentTrick = knowledge.getNoOfTricks()) != 0) {
			addTrickToPlayedCards(knowledge.getCompletedTricks().get(currentTrick-1));
		}
	...
	protected void addTrickToPlayedCards(Trick pTrick) {
		...
		if(tTrickSuit != pTrick.getSecondCard().getSuit()
				|| (tTrickSuit != knowledge.getTrumpSuit() && pTrick.getSecondCard().getRank() == Rank.JACK)) {
			if(pTrick.getMiddleHand() == knowledge.getPlayerPosition().getLeftNeighbor())
				oSituation.setLeftPlayerBlankColor(tTrickSuit);
			else if(pTrick.getMiddleHand() == knowledge.getPlayerPosition().getRightNeighbor())
				oSituation.setRightPlayerBlankColor(tTrickSuit);
		}
		else if(tTrickSuit == knowledge.getTrumpSuit()
				&& !(pTrick.getSecondCard().getSuit() == tTrickSuit || pTrick.getSecondCard().getRank() == Rank.JACK)) {
			if(pTrick.getMiddleHand() == knowledge.getPlayerPosition().getLeftNeighbor())
		...
		if(tTrickSuit != pTrick.getThirdCard().getSuit()
				|| (tTrickSuit != knowledge.getTrumpSuit() && pTrick.getThirdCard().getRank() == Rank.JACK)) {
			if(pTrick.getRearHand() == knowledge.getPlayerPosition().getLeftNeighbor())
				oSituation.setLeftPlayerBlankColor(tTrickSuit);
			else if(pTrick.getRearHand() == knowledge.getPlayerPosition().getRightNeighbor())
				oSituation.setRightPlayerBlankColor(tTrickSuit);
		}
		else if(tTrickSuit == knowledge.getTrumpSuit()
				&& !(pTrick.getThirdCard().getSuit() == tTrickSuit || pTrick.getThirdCard().getRank() == Rank.JACK)) {
			if(pTrick.getRearHand() == knowledge.getPlayerPosition().getLeftNeighbor())
				oSituation.setLeftPlayerBlankOnTrump();
			else if(pTrick.getRearHand() == knowledge.getPlayerPosition().getRightNeighbor())
				oSituation.setRightPlayerBlankOnTrump();
		...
		// Wenn Farbspiel
		if((knowledge.getGameType() == GameType.CLUBS || knowledge.getGameType() == GameType.SPADES || knowledge.getGameType() == GameType.HEARTS || knowledge.getGameType() == GameType.DIAMONDS)) {
			...
			if(tTrickSuit != knowledge.getTrumpSuit()
				...
				oSituation.setRightPlayerBlankColor(tTrickSuit);
					...
					&& oNotOpponentCards.getSuitCount(tTrickSuit, false)+Helper.countJacks(oNotOpponentCards) == 11) {
				oSituation.setLeftPlayerBlankOnTrump();
				oSituation.setRightPlayerBlankOnTrump();
			}
		}
...
//	}
	...
	protected static Card getRandomAllowedCard(final CardList cards, final Card initialCard, final GameType gameType) {
		Card result = null;
		for (Card c : cards) {
	...
	protected static Card getLowestBeatingCard(CardList pCards, GameType pGameType, Card tCardToBeat) {
		int counter	= -1;
		if(pGameType != GameType.GRAND)
			counter = pCards.getLastIndexOfSuit(tCardToBeat.getSuit(), false);
		if(counter == -1)
			counter = Helper.countJacks(pCards)-1;
		// Solange die niedrige Karte die Karte vom Solo-Spieler nicht besiegt, weiter hoch gehen im Ranking der Karten 
