...
public class AlgorithmSuit extends AbstractAlgorithmAI {
	...
	protected Card startGame() {
		System.out.println("Suit-Declarer starts Game: "+knowledge.getCurrentTrick().getForeHand());
	...
	protected Card playForehandCard() {
		System.out.println("Suit-Declarer plays Forehand-Card: "+knowledge.getCurrentTrick().getForeHand());
	...
	protected Card playMiddlehandCard() {
		System.out.println("Suit-Declarer plays Middlehand-Card: "+knowledge.getCurrentTrick().getMiddleHand());
	...
	protected Card playRearhandCard() {
		System.out.println("Suit-Declarer plays Rearhand-Card: "+knowledge.getCurrentTrick().getRearHand());
...
//		knowledge.removeOwnCards(tDiscardCards);
	...
	public static Card playStartGameCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation) {
		...
		// Count Aces
		int tAcesCount	= 0;
		if(pSituation.getTrumpSuit() != Suit.CLUBS		&& pCards.contains(Card.CA))	tAcesCount++;	// Clubs
		if(pSituation.getTrumpSuit() != Suit.SPADES		&& pCards.contains(Card.SA))	tAcesCount++;	// Spades
		if(pSituation.getTrumpSuit() != Suit.HEARTS		&& pCards.contains(Card.HA))	tAcesCount++;	// Hearts
		if(pSituation.getTrumpSuit() != Suit.DIAMONDS 	&& pCards.contains(Card.DA))	tAcesCount++;	// Diamonds
	...
	public static Card playForehandCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation) {
		...
		if(pNotOpponentCards.getSuitCount(pSituation.getTrumpSuit(), false) + Helper.countJacks(pNotOpponentCards) < 10) {
			...
			if(Helper.countJacks(pNotOpponentCards) == 4 
				...
				return pCards.get(0);
			// Wenn der Spieler die hoechste Trumpfkarte besitzt und der Gegner noch Trumpfkarten hat
			// oder alles bis auf eine Karte sind noch Trumpfkarten in der Hand des Spielers
			if(Helper.isHighestTrumpCard(pCards.get(0), pSituation.getGameType(), pPlayedCards)
				&& (pNotOpponentCards.getSuitCount(pSituation.getTrumpSuit(), false) != 7
					|| Helper.countJacks(pCards) + pCards.getSuitCount(pSituation.getTrumpSuit(), false) >= pCards.size()-1))
				return pCards.get(0);
			// sonst niedrigste Trumpfkarte zu den moeglichen Karten hinzufuegen
			// MÃƒÆ’Ã‚Â¶glichkeit der Karte erhoehen. 2x hinzufuegen
			possibleCards.add(pCards.get(pCards.getLastIndexOfSuit(pSituation.getTrumpSuit())));
			possibleCards.add(pCards.get(pCards.getLastIndexOfSuit(pSituation.getTrumpSuit())));
		}
		// Wenn ein Gegner noch Truempfe besitzt
		else if(pNotOpponentCards.getSuitCount(pSituation.getTrumpSuit(), false) + Helper.countJacks(pNotOpponentCards) == 10)
			possibleCards.add(pCards.get(pCards.getLastIndexOfSuit(pSituation.getTrumpSuit())));
		...
		for(Suit s : Suit.values()) {
			...
			Card possibleHighCard	= pCards.get(pCards.getFirstIndexOfSuit(s, false));	// highest Card
			Card possibleLowCard	= pCards.get(pCards.getLastIndexOfSuit(s, false));	// lowest Card
			
			if(possibleHighCard.equals(possibleLowCard))
				possibleCards.add(possibleLowCard);
			
			// Wenn der Spieler das Ass hat und weniger als 3 Karten dieser Farbe gespielt wurden
			// oder der Spieler die hoechste Karte der Farbe hat und kein Trumpf beim Gegner existiert
			if(possibleHighCard.getRank() == Rank.ACE && pPlayedCards.getSuitCount(s, false) < 3 
					|| (Helper.isHighestSuitCard(possibleHighCard, pPlayedCards, pTrickCards)
							&& Helper.countJacks(pNotOpponentCards)+pNotOpponentCards.getSuitCount(pSituation.getTrumpSuit(), false) == 11 ))
		...
		if(pCards.hasTrump(pSituation.getGameType())
				&& pCards.size() + pPlayedCards.getSuitCount(pSituation.getTrumpSuit(), false) + Helper.countJacks(pPlayedCards) == 11)
			return pCards.get(0);
	...
	public static Card playMiddlehandCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation) {
		...
		if(tForehandCard.getRank() == Rank.JACK
			...
			if(tForehandCard.getPoints() >= 10)
				return pCards.get(0);
					...
					&& Helper.isHighestTrumpCard(pCards.get(0), pSituation.getGameType(), pPlayedCards))
				return pCards.get(0);
			// Wenn Ass oder 10  noch im Spiel beim Gegner dann hohen Trumpf spielen
			else if((!pNotOpponentCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.ACE)) 
						|| (!pNotOpponentCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.TEN))))
		...
		if(tSuitCount == 1)
			return pCards.get(pCards.getFirstIndexOfSuit(tForehandCard.getSuit(), false));
		else if(tSuitCount > 1) {
			Card lCard = pCards.get(pCards.getFirstIndexOfSuit(tForehandCard.getSuit(), false));
			if(lCard.beats(pSituation.getGameType(), tForehandCard)
					&& Helper.isHighestSuitCard(lCard, pPlayedCards, pTrickCards)
					&& !pSituation.isLeftPlayerBlankOnColor(tForehandCard.getSuit()))
				return lCard;
			return pCards.get(pCards.getLastIndexOfSuit(tForehandCard.getSuit(), false));
		}
		
		// FOREHAND-Karte kann nicht bedient werden
		// Wenn die Karte >= 10 Punkte wert ist und der Spieler eine Trumpfkarte hat
		if(tForehandCard.getPoints() >= 10
				&& pCards.hasTrump(pSituation.getGameType())) {
			if(Helper.countJacks(pCards) > 0)
				return pCards.get(Helper.countJacks(pCards)-1);
			if(Helper.isHighestSuitCard(pCards.get(0), pSituation.getGameType(), pPlayedCards, pTrickCards))
				return pCards.get(0);
			return pCards.get(pCards.getLastIndexOfSuit(pSituation.getTrumpSuit()));
		}
		...
		for(Suit s : Suit.values()) {
			...
			// Wenn nur eine Karte dieser Farbe, die zudem <= 4 Punkte Wert ist und eine Karte mit Wert >= 10 noch im Spiel ist -> abwerfen 
			if(pCards.getSuitCount(s, false) == 1 && possibleHighCard.getPoints()<=4
					&& (!pNotOpponentCards.contains(Card.getCard(s, Rank.ACE)) || !pNotOpponentCards.contains(Card.getCard(s, Rank.TEN))))
				return possibleHighCard;
			// Wenn 3 Karten einer Farbe, dann kann die niedrigste Karte geworfen werden
			// oder 2 Karten und keine hohe Karte dabei
			// oder 2 Karten und hohe Karte ist die hoechste 
				...
				// Wenn Ass & <=Queen
					...
					possibleCards.add(possibleLowCard);
				else if(Helper.isHighestSuitCard(possibleHighCard, pSituation.getGameType(), pPlayedCards, pTrickCards))
					possibleCards.add(possibleLowCard);
		...
		if(pCards.hasTrump(pSituation.getGameType())) {
			if(!pSituation.isLeftPlayerBlankOnColor(tForehandCard.getSuit())
					|| pPlayedCards.getSuitCount(tForehandCard.getSuit(), false) < 4)
	...
	public static Card playRearhandCard(CardList pCards, CardList pTrickCards, CardList pPlayedCards, CardList pNotOpponentCards, Situation pSituation) {
		...
		if(tForehandCard.getRank() == Rank.JACK
			...
			if(tMiddlehandCard.getPoints() + tForehandCard.getPoints() >= 7) {
				...
				if(Helper.getTrumpCardsToBinary(pCards, pSituation.getTrumpSuit()) > tCardToBeat.getRank().toBinaryFlag()) {
					// Spiele den niedrigsten hoeheren Trumpf
					for(int i=pCards.getLastIndexOfSuit(pSituation.getTrumpSuit()); i>=0; i--) {
						if(pCards.get(i).beats(pSituation.getGameType(), tCardToBeat))
							return pCards.get(i);
					}
				}
			}
			// Wenn der Spieler das Trumpf-Ass hat und mit dem Ass gewinnt
			if(pCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.ACE)) && Card.getCard(pSituation.getTrumpSuit(), Rank.ACE).beats(pSituation.getGameType(), tCardToBeat))
				return Card.getCard(pSituation.getTrumpSuit(), Rank.ACE);
			// Wenn der Spieler die Trumpf-10 hat und mit dem Ass gewinnt
			if(pCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.TEN)) && Card.getCard(pSituation.getTrumpSuit(), Rank.TEN).beats(pSituation.getGameType(), tCardToBeat))
		...
		if(tMiddlehandCard.getPoints() + tForehandCard.getPoints() >= 8) {
			...
			if(tCardToBeat.getSuit() == pSituation.getTrumpSuit()
					&& Helper.getTrumpCardsToBinary(pCards, pSituation.getTrumpSuit()) > tCardToBeat.getRank().toBinaryFlag())
				// Spiele den niedrigsten hoeheren Trumpf
				for(int i=pCards.getLastIndexOfSuit(pSituation.getTrumpSuit()); i>=0; i--) {
					if(pCards.get(i).beats(pSituation.getGameType(), tCardToBeat))
						return pCards.get(i);
			...
			if(pCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.KING)) && Card.getCard(pSituation.getTrumpSuit(), Rank.KING).beats(pSituation.getGameType(), tCardToBeat))
		...
		for(Suit s : Suit.values()) {
			if(s == pSituation.getTrumpSuit() || pCards.getSuitCount(s, false) == 0)
				continue;
			// Wenn nur eine Karte dieser Farbe, die zudem <= 4 Punkte Wert ist und eine Karte mit Wert >= 10 noch im Spiel ist -> abwerfen 
			if(pCards.getSuitCount(s, false) == 1 && pCards.get(pCards.getFirstIndexOfSuit(s)).getPoints()<=4
					&& (!pNotOpponentCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.ACE)) || !pNotOpponentCards.contains(Card.getCard(pSituation.getTrumpSuit(), Rank.TEN))))
		...
		if(pCards.getTrumpCount(pSituation.getTrumpSuit()) > 0) {
			if(pCards.getSuitCount(pSituation.getTrumpSuit(), false) > 0)
				return pCards.get(pCards.getFirstIndexOfSuit(pSituation.getTrumpSuit()));
			return pCards.get(Helper.countJacks(pCards)-1);
		}
		...
		return getRandomAllowedCard(possibleCards, tForehandCard, pSituation.getGameType());
	}
	...
	public static CardList discardSkatCards(BidEvaluator pBid, CardList pOwnCards) {
		...
		for(Suit lSuit : Suit.values()) {
			if(lSuit == pBid.getSuggestedTrumpSuit() || tCards.getSuitCount(lSuit, false) == 0)
				continue;
			...
			Card lLastCard	= tCards.get(tCards.getLastIndexOfSuit(lSuit, false));
		...
		if(tDiscardCards.size() != 2 && !tTenZeroToDiscard.isEmpty()) {
			...
			for(CardList cards : tTenZeroToDiscard) {
				if(cards.get(0).getPoints()+cards.get(1).getPoints() > points) {
					points = cards.get(0).getPoints()+cards.get(1).getPoints();
		...
		if(tDiscardCards.size() == 0 && tCards.getTrumpCount(pBid.getSuggestedTrumpSuit()) == 11) {
			tDiscardCards.add(tCards.get(tCards.size()-1));
		}
		if(tDiscardCards.size() == 1 && tCards.getTrumpCount(pBid.getSuggestedTrumpSuit()) == 11) {
