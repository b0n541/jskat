...
public class AlgorithmRamsch extends AbstractAlgorithmAI {
    ...
    AlgorithmRamsch(final AlgorithmAI p, final GameType pGameType) {
        ...
        log.debug(String.format("/s is %s", myPlayer.getPlayerName(), getClass().getName()));
    }
    ...
    public Card startGame() {
        ...
        for (final Suit s : Suit.values()) {
            if (cards.getSuitCount(s, false) == 1) {
                if (resultIndex < 0
                        || cards.get(resultIndex).getSuitGrandOrder() > cards.get(
                        cards.getFirstIndexOfSuit(s)).getSuitGrandOrder()) {
    ...
    public Card playForehandCard() {
        ...
        for (final Suit s : Suit.values()) {
            ...
            if (cards.getSuitCount(s, false) == 1
                    && c.getRank().getSuitGrandOrder() < 6
                    && c.getRank().getSuitGrandOrder() > 2) {
                if (resultIndex < 0) {
                    resultIndex = cards.getIndexOf(c);
                } else {
                    if (c.getSuitGrandOrder() > cards.get(resultIndex)
                            .getSuitGrandOrder()) {
                        resultIndex = cards.getIndexOf(c);
                    }
                }
     ...
     */
    private static Card getDefaultCard(final CardList cards, final Card initialCard,
    ...
    public CardList discardSkat(final BidEvaluator bidEvaluator) {
        log.debug(myPlayer.getPlayerName() + " (" + getClass()
                + ") is discarding cards");
